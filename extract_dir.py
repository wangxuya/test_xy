...
caffe_root = '/data/public/caffe0925/'
...
def get_feature(img_file):
    ...
    net.blobs['data'].data[...] = transformer.preprocess('data', caffe.io.load_image(img_file))
    net.forward()
    conv5 = net.blobs['conv5'].data[0,]
    c = np.reshape(conv5, (conv5.shape[0], conv5.shape[1]*conv5.shape[2]))
    means = c.mean(axis=1)
    return means
...
#            print path.join(parent,filename)
            ...
            try:
                ...
                f = open(img_filename + '.cnn_conv5', 'wb')
                f.write(feature)
                f.close()
            except:
                print img_filename
...
#extract_dir('/home/wxy/Holidays/images/')
extract_dir('/home/wxy/data1/UK_bench/2/')
